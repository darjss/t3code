...
- **Passed:** 1a (validation + runtime errors surface as full rows), 2 (stop → "You stopped after 18s", turn state `interrupted`), 3 (tool start/end rows, no churn), 4a/4b (context meter 3.1% → 4.7%, accumulates), 5a/6a (compact button → pi compacts — session file gains a compaction entry; the UI row required a fix, see below), 6b (only T3-native `/model`, no pi slash commands), 6c (nous model selection + run), session resume after server restart ("Hi — the session resumed fine.").
