...
/** The timestamp a settled row sorts and labels by. The server stamps
    settledAt on every settle — user settles stamp the command time and
    auto-settles backdate to the thread's last activity — so it directly
    reads "when the work ended". updatedAt is the net for malformed data. */
