...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("keeps the T3-Pi product name across update channels", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "T3-Pi Code");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3-Pi Code");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3-Pi Code (Alpha) 1.2.3 Installer",
      ...
      // Linux must register its own renderer scheme so OAuth deep links do not
      // replace the installed Nightly app's handler.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3-Pi Code", schemes: ["t3code-pi"] },
      ]);
      assert.equal((linux.linux as Record<string, unknown>).executableName, "t3-pi-code");
      assert.equal(linux.artifactName, "T3-Pi-Code-${version}-${arch}.${ext}");
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.t3tools.t3code.pi</string>");
  ...
  it.effect("adds passkey entitlements and the T3-Pi protocol to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.protocols, [{ name: "T3-Pi Code", schemes: ["t3code-pi"] }]);
    }).pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })))),
  );
